...
], (Controller, ODataModel, MessageToast, Filter, FilterOperator, BusyIndicator) => {
    "use strict";

    return Controller.extend("zadvpaymupd.controller.Upload", {
        onInit() {
        ...
        onClickPost() {
            ...
            let data = selectedData
                .map((element) => {
                    ...
                    return {
                        Companycode: data1.Companycode,
                        Documentdate: data1.Documentdate,//.toISOString().replace("T00:00:00.000Z", "").replace(/-/g, ""),
                        Bpartner: data1.Bpartner
                    }
                })
                .filter((item) => {
                    const key = `${item.Companycode}_${item.Documentdate}_${item.Bpartner}`;
            ...
            $.ajax({
                url: '/sap/bc/http/sap/ZHTTP_ADVPAYMPOST',
                method: "POST",
                        ...
                        workbook.SheetNames.forEach(function (sheetName) {
                            ...
                            excelData.forEach((element) => {
                                ...
                                datas.push({
                                    ...
                                    SpecialGLCode: element["Special GL Indicator"]
                                });
                            });

                            BusyIndicator.show();
                            $.ajax({
                                url: '/sap/bc/http/sap/ZHTTP_ADVANCEPAYM',
                                method: "POST",
                                contentType: "application/json",
        ...
        beforerRebind(e) {
            ...
            aDateFilters.push(new Filter("Type", FilterOperator.EQ, 'ADVC'))
